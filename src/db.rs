...
pub struct LRUCache<K, V> where K: Clone + Eq + Hash {
    pub capacity: usize,
    pub cache: Arc<Mutex<HashMap<K, (V, Instant)>>>,
    pub order: Arc<Mutex<VecDeque<K>>>,
}
...
impl<K: Eq + Hash + Clone + Default + Send + 'static, V: Default + Send + 'static> LRUCache<K, V> {
        ...
        thread::spawn(move || {
            loop {
                ...
                let keys_to_remove: Vec<K> = cache.iter()
                    .filter(|(key, (_, creation_time))| Instant::now().duration_since(*creation_time) > Duration::from_secs(10))
                    .map(|(key, _)| key.clone())
                    .collect();
    ...
    pub fn put(&mut self, key: K, val: V, time_based: bool) {
    ...
    pub fn is_expired(&self, key: &K, max_age: Duration) -> bool {
